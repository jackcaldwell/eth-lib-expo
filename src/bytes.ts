...
const random = async (bytes: number): Promise<string> => {
  const { getRandomBytesAsync } = Random;

  let hex = '0x';
  for (let i = 0; i < bytes; ++i)
    hex += ('00' + (await getRandomBytesAsync(i).toString())).slice(-2);
  return hex;
};
...
const toArray = (hex: string): number[] => {
  let arr = [];
...
const fromUint8Array = (arr: Uint8Array): string =>
...
const fromString = (s: string): string | null => {
  const makeByte = (uint8: number): string => {
...
const toString = (bytes: string): string | null => {
  let s = '';
  let i = 0;
  let l = length(bytes);
  while (i < l) {
